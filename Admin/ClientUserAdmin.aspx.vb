...
Public Class ClientUserAdmin
    ...
    Protected Sub clearForm()
        ...
        '    RoleCheckBox.Checked = (RoleCheckBox.Text = "Client")
        'Next
    ...
    Private Sub RadGrid1_ItemCommand(ByVal source As Object, ByVal e As Telerik.Web.UI.GridCommandEventArgs) Handles RadGrid1.ItemCommand
        If e.CommandName = "RowClick" Then
            ...
            loadUserLocationSelectList()
            pnlClientList.Visible = cuser.myRoles(0) = "Client"
            pnlLocations.Visible = (cuser.myRoles(0) = "Client" Or cuser.myRoles(0) = "Guest")
            'cuser.myRoles(0) = "Client"
    ...
    Public Sub loadClientList()
        ...
        Dim dt As DataTable = ldal.getParentCompanies()
        cbClientList.DataSource = dt
        cbClientList.DataTextField = "Name"
        cbClientList.DataValueField = "ID"
        cbClientList.DataBind()
    End Sub
    ...
    Public Sub chkLocaList(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Select Case rlc.Checked
            Case True
                Select Case rlc.Text
                    Case "Client"
                        rlstr = "Vendor Carrier Guest"
                    Case "Vendor"
                        rlstr = "Client Carrier Guest"
                    Case "Carrier"
                        rlstr = "Client Vendor Guest"
                    Case "Guest"
                        rlstr = "Client Vendor Carrier"
                End Select
        End Select
        ...
        For Each ri As RepeaterItem In UsersRoleList.Items
            ...
            End If
        Next
        If Not rlst.Contains("Client") Then
            cbClientList.ClearSelection()
            cbClientList.Text = ""
            pnlClientList.Visible = False
            loadUserLocationSelectList()
        Else
            pnlClientList.Visible = True
        End If
        pnlLocations.Visible = (rlst.Contains("Client") Or rlst.Contains("Guest"))
    End Sub

    Private Sub loadUserLocaList(ByVal usr As ssUser)
        Dim ldal As New locaDAL
        Dim dt As DataTable = ldal.getUserLocaList(usr.userID)
        lbLocationList.Items.Clear()
        If dt.Rows.Count > 0 Then
            lbLocationList.DataSource = dt         'locaID, LocationName
            lbLocationList.DataTextField = "LocationName"
            lbLocationList.DataValueField = "locaID"
    ...
    Private Sub loadUserLocationSelectList()
        ...
        If cbClientList.SelectedIndex > -1 Then
            Dim pcID As Guid = New Guid(cbClientList.SelectedValue.ToString)
            dt = ldal.getLocationsByParentID(pcID)
        Else
            dt = ldal.getLocations  'returns fields: locaID, LocationName, ParentCompanyID, ParentCompanyName
        End If
        lbAvailableLocations.Items.Clear()
        lbAvailableLocations.DataSource = dt
        lbAvailableLocations.DataValueField = "locaID"
        lbAvailableLocations.DataTextField = "LocationName"
    ...
    Private Sub RadGrid1_NeedDataSource(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles RadGrid1.NeedDataSource
        ...
        If selRole = "Client" Or selRole = "Vendor" Or selRole = "Carrier" Or selRole = "Guest" Then
            gusers = udal.getUsersByRole(selRole)
        Else
            gusers = udal.getClientServicesUsers()
        End If
...
#Region "Roles"
    ...
    Private Sub BindRolesToList()
        ...
        Dim rls() As String = Split("Client Vendor Carrier Guest")
...
#End Region
            ...
            Select Case e.CommandName
                Case "Insert"
                    ...
                    If strResponse = "The user account was successfully created!" Then
                        nUser = udal.getUserByName(nUser.userName)
                        btnSubmit.CommandArgument = nUser.userID.ToString
                        If nUser.myRoles.Contains("Guest") Or nUser.myRoles.Contains("Client") Then 'populate the location selectors.
                            Dim dba As New DBAccess
                            For Each it As RadListBoxItem In lbLocationList.Items
                                ...
                                dba.AddParameter("@userID", nUser.userID)
                ...
                Case "Update"
                    ...
                    If updatedUser.IsApproved Then updatedUser.LastActivityDate = Date.Now.AddDays(-5)
                    ...
                    If updatedUser.myRoles.Contains("Guest") Or updatedUser.myRoles.Contains("Client") Then 'populate the location selectors.
                        For Each it As RadListBoxItem In lbLocationList.Items
                            dba.CommandText = "INSERT INTO UserLocations (userID, locationID) VALUES (@userID, @locationID)"
                            dba.AddParameter("@userID", updatedUser.userID)
                            dba.AddParameter("@locationID", New Guid(it.Value))
    ...
    Private Sub cbClientList_SelectedIndexChanged(ByVal sender As Object, ByVal e As Telerik.Web.UI.RadComboBoxSelectedIndexChangedEventArgs) Handles cbClientList.SelectedIndexChanged
        loadUserLocationSelectList()
    End Sub
End Class