...
Public Class AddEditTimePunche
    ...
    End Sub
#Region "GridTIO"
    Private Sub gridTIO_NeedDataSource(sender As Object, e As GridNeedDataSourceEventArgs) Handles gridTIO.NeedDataSource
        Dim dba As New DBAccess
        dba.CommandText = "SELECT TimeInOut.ID, TimeInOut.TimeIn, TimeInOut.TimeOut, JobDescription AS JobDescription, TimeInOut.JobDescriptionID " & _
            "FROM TimeInOut LEFT OUTER JOIN JobDescriptions ON TimeInOut.JobDescriptionID = JobDescriptions.ID " & _
            "WHERE (TimeInOut.TimepuncheID = @TimepuncheID) ORDER BY TimeInOut.TimeIn"
        dba.AddParameter("@TimepuncheID", Session("TimepuncheID"))
        Dim dt As DataTable = dba.ExecuteDataSet.Tables(0)
        gridTIO.DataSource = dt
    End Sub
    ...
    Private Sub gridTIO_ItemInserted(sender As Object, e As GridInsertedEventArgs) Handles gridTIO.ItemInserted
        If TypeOf e.Item Is GridEditFormInsertItem And gridTIO.MasterTableView.IsItemInserted Then
            Dim editform As GridEditFormInsertItem = e.Item
            Dim jobdescription As RadComboBox = CType(editform.FindControl("cbJobdescription"), RadComboBox)
            Dim dba As New DBAccess
            dba.CommandText = "SELECT LocationJobDescriptions.JobDescriptionID, JobDescriptions.JobDescription FROM LocationJobDescriptions INNER JOIN JobDescriptions ON LocationJobDescriptions.JobDescriptionID = JobDescriptions.ID WHERE (LocationJobDescriptions.LocationID = @locaid)"
            dba.AddParameter("@locaid", Session("locaid"))
            Dim dt As DataTable = dba.ExecuteDataSet.Tables(0)
            jobdescription.DataSource = dt
            jobdescription.DataTextField = "JobDescription"
            jobdescription.DataValueField = "JobDescriptionID"
    ...
    Private Sub gridTIO_ItemDataBound(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridItemEventArgs) Handles gridTIO.ItemDataBound
        If (TypeOf e.Item Is GridEditableItem AndAlso e.Item.IsInEditMode) Then
            ...
            Dim jobdescription As RadComboBox = CType(item.FindControl("cbJobdescription"), RadComboBox)
        

            Dim utl As New Utilities
            If Utilities.IsValidGuid(Session("locaid").ToString) Then
                Dim locaid As Guid = New Guid(Session("locaid").ToString)
            End If
    ...
    Private Sub gridTIO_UpdateCommand(ByVal sender As Object, ByVal e As GridCommandEventArgs) Handles gridTIO.UpdateCommand
        ...
        If (TypeOf e.Item Is GridEditFormItem AndAlso e.Item.IsInEditMode) Then
            ...
            For i As Integer = 0 To etp.tpList.Count - 1
                ...
                jobdesc = CType(editFormItem.FindControl("cbjobdescription"), RadComboBox)

                tio.JobDescriptionID = New Guid(jobdesc.SelectedValue)
                tio.isHourly = utl.isHourly(tio.JobDescriptionID)
                tio.TimeIn = sDate.SelectedDate
                tio.TimeIn = etp.DateWorked.ToShortDateString & " " & tio.TimeIn.ToShortTimeString
                If eDate.SelectedDate Is Nothing Then
        ...
        updateStr = tpdal.updateTIO(tio)
        Try
            gridTIO.MasterTableView.ClearEditItems()
            gridTIO.Rebind()
        Catch ex As Exception
