...
Public Class AvgUnloadTimePerVendor
    ...
    Private Sub RadGrid1_NeedDataSource(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles RadGrid1.NeedDataSource
        ...
        dba.CommandText = "SELECT dbo.Vendor.Name, dbo.WorkOrder.VendorNumber AS [VendorNumber], CONVERT(VARCHAR(5),  " & _
            "CAST(AVG(CAST(dbo.WorkOrder.CompTime - dbo.WorkOrder.StartTime AS float)) AS datetime), 108) AS AvgTime " & _
            ...
            "dbo.ParentCompany ON dbo.ParentCompany.ID = dbo.Vendor.ParentCompanyID " & _
            "WHERE (dbo.Location.Name = @Location) AND (WorkOrder.LogDate >= @FromLogDate) AND (WorkOrder.LogDate <= DATEADD(n, 1439, @ToLogDate)) " & _
            "AND (CONVERT(Varchar(5), dbo.WorkOrder.CompTime - dbo.WorkOrder.StartTime, 108) <> '00:00') " & _
            "GROUP BY dbo.WorkOrder.VendorNumber, dbo.Vendor.Name "
