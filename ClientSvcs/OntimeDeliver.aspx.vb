...
Public Class OntimeDeliver
    ...
    Private Sub RadGrid1_NeedDataSource(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles RadGrid1.NeedDataSource
        ...
        dba.CommandText = "SELECT l.Name AS Location, d.Name AS Department, v.Name AS Vendor, wo.PurchaseOrder AS [PONumber], " & _
            "wo.DockTime AS [DockTime], wo.AppointmentTime AS [AppointmentTime],  " & _
            "(CASE WHEN wo.DockTime > DATEADD(MINUTE, 15, wo.AppointmentTime) THEN 'NO' ELSE 'YES' END) AS [OnTime] " & _
            "FROM dbo.WorkOrder AS wo INNER JOIN " & _
            "dbo.Location AS l ON l.ID = wo.LocationID INNER JOIN " & _
            "dbo.Department AS d ON d.ID = wo.DepartmentID INNER JOIN " & _
            "dbo.Vendor AS v ON v.ID = wo.CustomerID INNER JOIN " & _
            "dbo.ParentCompany AS pc ON pc.ID = v.ParentCompanyID " & _
            "WHERE (d.Name <> 'BJ Wholesale') AND (l.Name = @Location) AND (DATEPART(HH, wo.DockTime) = 5 OR " & _
            "DATEPART(HH, wo.DockTime) = 6) AND (wo.TruckNumber <> 'DROP') AND (wo.LogDate >= @FromLogDate) AND (wo.LogDate <= DATEADD(n, 1439, @ToLogDate))" & _
            "ORDER BY wo.LogDate, Vendor, Location, Department "
